...
public class MenuBarAppli extends MenuBar {
	...
	public void init() {
		...
		item = new MenuItem( "Candidats" );
		item.setOnAction(  (e) -> managerGui.showView( EnumView.CandidatListe )  );
		menu.getItems().add( item );
		itemComptes = item;
...
//				managerReport.print( EnumReport.AnnuaireTelephone, null ) );
		...
		item = new MenuItem( "DaoCandidat" );
		item.setOnAction(  (e) -> managerGui.showView( EnumView.TestDaoCandidat )  );
		menu.getItems().add( item );

		item = new MenuItem( "DaoCategorie1" );
		item.setOnAction(  (e) -> managerGui.showView( EnumView.TestDaoCategorie1 )  );
		menu.getItems().add( item );
